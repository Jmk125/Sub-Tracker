...
app.post('/api/ai/parse-quote', upload.single('quotePdf'), async (req, res) => {
  ...
  try {
    ...
    const prompt = 'Extract subcontractor quote contact data as JSON with keys: company_name,contact_name,contact_phone,contact_email,address,city,state,zip,notes,division_num. Return only valid JSON.';
    ...
    const textOut = aiData.output_text || '';
    let fields = {};
    try { fields = JSON.parse(textOut); } catch (e) {
